...
// Install this plugin into the given directory.
...
module.exports = {
  ...
  createSnsTopic(topicName, endpoint=undefined) {
    const SNS = new AWS.SNS({ region: 'us-east-1', endpoint: endpoint });
  ...
  removeSnsTopic(topicName, endpoint=undefined) {
    const SNS = new AWS.SNS({ region: 'us-east-1', endpoint: endpoint });
    BbPromise.promisifyAll(SNS, { suffix: 'Promised' });
  ...
  publishSnsMessage(topicName, message, endpoint=undefined) {
    const SNS = new AWS.SNS({ region: 'us-east-1', endpoint: endpoint });
    BbPromise.promisifyAll(SNS, { suffix: 'Promised' });
